...
def train_epoch(epoch, model, optimizer, criterion, loader, num_classes, device):
    ...
    acc_metric = torchmetrics.Accuracy(task="multiclass", threshold = 0.5, num_classes = 7, average = 'macro').to(device)
    rec_metric = torchmetrics.Recall(task="multiclass", threshold = 0.5, num_classes = 7, average = 'macro').to(device)
...
def val_epoch(epoch, model, criterion, loader, num_classes, device):
    ...
    confusion_matrix = torchmetrics.ConfusionMatrix(task="multiclass", num_classes = 7).to(device)
